...
vitisFromADCPipe="rx_demo_input_bundle_1_inst1"
vitisFromRxPipe="rx_demo_output_bundle_1_inst1"

vitisToTxPipe="tx_demo_input_bundle_1_inst1"
vitisToDACPipe="tx_demo_output_bundle_1_inst1"
...
mkdir rx
cd rx
tmux new-session -d -s vitis_cyclops_demo_inst1 "printf '\\033]2;%s\\033\\\\' 'Rx_DSP'; ${RxDir}/benchmark_rx_demo_io_posix_shared_mem"
tmux rename-window -t vitis_cyclops_demo_inst1:0 'vitis_cyclops_demo_inst1'
tmux set-option -t vitis_cyclops_demo_inst1 pane-border-status top

echo "${RxDir}/benchmark_rx_demo_io_posix_shared_mem"
cd ..
mkdir tx
cd tx
tmux split-window -h -d -t vitis_cyclops_demo_inst1:0 "printf '\\033]2;%s\\033\\\\' 'Tx_DSP'; ${TxDir}/benchmark_tx_demo_io_posix_shared_mem"
# tmux rename-window -t vitis_cyclops_demo_inst1:1 'tx'
echo "${TxDir}/benchmark_tx_demo_io_posix_shared_mem"
cd ..

#Create Feeback Pipe for the Application Layer Side
TxFeedbkAppPipeName="txFeedbkAppLayer_inst1"
...
tmux split-window -v -d -t vitis_cyclops_demo_inst1:0 "printf '\\033]2;%s\\033\\\\' 'Cyclops_ASCII_Application'; ${cyclopsASCIIBuildDir}/cyclopsASCIILink -rx ${vitisFromRxPipe} -tx ${vitisToTxPipe} -txfb ${TxFeedbkAppPipeName} -txtokens ${TxTokens} -cpu ${appCPU} -processlimit ${ProcessLimitCyclops} -txdutycycle ${txdutycycle} -rxsubsampleperiod ${rxsubsampleperiod} -fifosize ${IO_FIFO_SIZE}"
# tmux rename-window -t vitis_cyclops_demo_inst1:2 'cyclopsASCII'
...
if [ ${USE_DUMMY} -ne 0 ]; then
    #Start dummyAdcDac
    tmux split-window -v -d -t vitis_cyclops_demo_inst1:0.2 "printf '\\033]2;%s\\033\\\\' 'Dummy_DAC/DAC'; ${dummyAdcDacBuildDir}/dummyAdcDacSharedMemFIFO -cpu ${txCPU} -rx ${vitisFromADCPipe} -tx ${vitisToDACPipe} -txfb ${TxFeedbkAppPipeName} -blocklen ${BlockSize} -fifosize ${IO_FIFO_SIZE}"
    # tmux rename-window -t vitis_cyclops_demo_inst1:3 'dummyAdcDac'
...
if [ -z ${TMUX} ]; then
    tmux attach-session -t vitis_cyclops_demo_inst1
else
    echo "Use TMUX to switch to the new session: vitis_cyclops_demo_inst1"
fi
