...
vitisFromADCPipe="rx_demo_input_bundle_1_inst2"
vitisFromRxPipe="rx_demo_output_bundle_1_inst2"

vitisToTxPipe="tx_demo_input_bundle_1_inst2"
vitisToDACPipe="tx_demo_output_bundle_1_inst2"
...
mkdir rx
cd rx
tmux new-session -d -s vitis_cyclops_demo_inst2 "printf '\\033]2;%s\\033\\\\' 'Rx_DSP'; ${RxDir}/benchmark_rx_demo_io_posix_shared_mem"
...
mkdir tx
cd tx
tmux split-window -h -d -t vitis_cyclops_demo_inst2:0 "printf '\\033]2;%s\\033\\\\' 'Tx_DSP'; ${TxDir}/benchmark_tx_demo_io_posix_shared_mem"
...
if [ ${USE_DUMMY} -ne 0 ]; then
    #Start dummyAdcDac
    tmux split-window -v -d -t vitis_cyclops_demo_inst2:0.2 "printf '\\033]2;%s\\033\\\\' 'Dummy_DAC/DAC'; ${dummyAdcDacBuildDir}/dummyAdcDacSharedMemFIFO -cpu ${txCPU} -rx ${vitisFromADCPipe} -tx ${vitisToDACPipe} -txfb ${TxFeedbkAppPipeName} -blocklen ${BlockSize} -fifosize ${IO_FIFO_SIZE}"
