...
if [[ -z ${parentPID} ]]; then
    ...
    pkill -P ${parentPID} -f benchmark_tx_demo_io_linux_pipe --signal SIGSTOP
    pkill -P ${parentPID} -f benchmark_rx_demo_io_linux_pipe --signal SIGSTOP
    pkill -P ${parentPID} -f benchmark_tx_demo_io_posix_shared_mem --signal SIGSTOP
    pkill -P ${parentPID} -f benchmark_rx_demo_io_posix_shared_mem --signal SIGSTOP
